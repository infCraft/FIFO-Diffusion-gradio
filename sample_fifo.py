...
from opensora_fifo.sample.pipeline_videogen import VideoGenPipeline
...
def main(args):
    ...
    for prompt in args.text_prompt:
        ...
        print(f"num_vae = {num_vae}")
        # num_vae必须大于0才可以保存视频。也就是必须大于56。
        ...
        if num_vae > 0:
            ...
            # fifo_vae_video_frames = fifo_vae_video_frames.cpu()
            imageio.mimwrite(output_vae_path, fifo_vae_video_frames, fps=args.fps, quality=9)
    
    return output_vae_path, None

def craft(prompt, seed, video_length, version):
    if not prompt:
        return None, "Error: prompt is empty! "
    parser = argparse.ArgumentParser()

    # 可调节参数
    parser.add_argument("--version", type=str, default=version, choices=[None, '65x512x512', '221x512x512', '513x512x512'])
    parser.add_argument("--new_video_length", "-N", type=int, default= video_length * 100 // 16)
    ...
    parser.add_argument("--text_prompt", default=prompt)
    parser.add_argument('--force_images', action='store_true', default=True)
    parser.add_argument('--tile_overlap_factor', type=float, default=0.25)
    parser.add_argument('--enable_tiling', action='store_true')
    parser.add_argument("--cache_dir", type=str, default="opensoraplan_models/")
    ...
    parser.add_argument("--num_partitions", "-n", type=int, default=8)

    args = parser.parse_args()

    assert args.num_frames == 4*args.video_length - 3
    
    if version == '221x512x512':
        args.num_frames = 221
        args.video_length = 56
        args.num_partitions = 4
        args.new_video_length = video_length * 256 // 36
    
    if args.new_video_length < args.video_length:
        args.new_video_length = args.video_length

    args.queue_length = args.video_length * args.num_partitions
    args.num_sampling_steps = args.video_length * args.num_partitions

    print(f"video_length = {args.new_video_length}")
